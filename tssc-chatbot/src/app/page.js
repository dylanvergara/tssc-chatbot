...
FOUNDER & COMMUNITY EXPLAINER VIDEO TRANSCRIPT — use this to answer any questions about how TSSC works, who Dylan is, pricing, guarantees, the process, FAQs, etc.:
...
On the guarantee: Dylan cannot guarantee income because he cannot apply to jobs, show up to them, or do the work for members. The guarantee is that if a member checks all five criteria listed in the agreement, within 60 days they will have an offer extended that has a validated on-target earnings matching or exceeding what they paid to join. Of the last 100 high-ticket paying clients, there have been no refunds and no chargebacks. If someone completes everything required and does not get the result, Dylan will work with them one-on-one Monday through Friday until they do.

On refunds: if someone joins and immediately wants out, they get a full refund. If they have used the product — had one-on-one time with Dylan, had their resume rebuilt, applied to jobs from the hiring channel — they are not eligible for a refund. They are eligible for the one-on-one support until they get the result.

On pricing: the community is not free and not cheap. Pricing is shared on the booking call. Payment plans are available, including options that do not require a credit check. With a credit score and some income, members have been able to get down to around $147/month. No early payoff penalties on that option. Lifetime access is available and the most discounted rate is presented on the initial call, the second most discounted at end of the 6 months, and the highest price if someone leaves and comes back later. There are no discounts for bringing a friend, but referral fees are paid: $500 if the referrer is an active member, $250 if not.
...
On success rate: for people who complete the five criteria in the agreement, Dylan describes it as 100%. Of the last 100 high-ticket paying clients, no refunds, no chargebacks.
...
To get started: book a 45-minute call via the booking link. Fill out the typeform. After the call, if it is a good fit, there is an agreement, payment, and onboarding. From agreement to being inside the community takes less than 10 minutes. Pricing is shown on the booking form at serialsalescommunity.co.`;
...
export default function ChatPage() {
  ...
  const handleSuggestion = (s) => {
    if (s.prompt === 'tssc_question') {
      setMessages([{ role: 'assistant', content: "What would you like to know about The Serial Sales Community? I can cover the program structure, what\'s included, the guarantee, pricing, and more." }]);
